<?php

ini_set('display_errors', 1);
ini_set('error_reporting', -1);

session_start();

// нужно добавить защиту от вызова извне.
//
if (!isset($_SESSION))
{
    $rs = ['success' => 0, 'message' => 'Start without session!'];
    die(json_encode($rs));
}

//
if (!array_key_exists('app_start', $_SESSION)){
    $rs = ['success' => 0, 'message' => 'Start without session!'];
    die(json_encode($rs));
}

// нужно добавить защиту от единовременного вызова


$dir = __DIR__ . '/../fonts/arial.ttf';

require '../php/functions.php'; 

// 
$captcha_item_count = 6; 
$words = '0123456789qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM'; 
$captcha_text = '';

for($i=1; $i<=6; $i++){	
	$captcha_numbers = rand(1,9);
	$captcha_text .= $words[rand(0,strlen($words)-1)];
}

$_SESSION['captcha'] = $captcha_text;

$all_colors = json_decode(file_get_contents('all_colors.json'));
//echo Debug::d($all_colors,'');
$rand_color = $all_colors[rand(0,count($all_colors)-1)];

// for($i=0;$i<=100;$i++){
// $rand_color = $all_colors[rand(0,count($all_colors)-1)];
// echo Debug::d($rand_color,'');
// }

//
function convert_hexcolor2dec($color)
{
	// $rand_color
	// we have something like this --> @34Feac
	// we need to convert it to 200,100,90
	// 3 двойки нужно разбить 
		
	$fd = hexdec(mb_substr($color,1,2));
	$sd = hexdec(mb_substr($color,3,2));
	$th = hexdec(mb_substr($color,5,2));
	// echo Debug::d(($fd),'');
	// echo Debug::d(($sd),'');
	// echo Debug::d(($th),'');
	return [$fd, $sd, $th];
}
$fixed_color = "#B56ADA"; 
$main_color = convert_hexcolor2dec($fixed_color);

$img['width']  = 85;
$img['height'] = 29;
$img['resourse'] = imagecreatetruecolor($img['width'], $img['height']);

$rand_color_converted = imagecolorallocate($img['resourse'], $main_color[0], $main_color[1], $main_color[2]); 
$black = imagecolorallocate($img['resourse'], 0, 0, 0);
$color = imagecolorallocate($img['resourse'], 200, 100, 90); // red
$white = imagecolorallocate($img['resourse'], 255, 255, 255);

imagefilledrectangle($img['resourse'],0,0,399,99,$white);

// imagettftext ( resource $img['resourse'] , float $size , float $angle , int $x , int $y , int $color , string $fontfile , string $text ) : array

$angle_array = [];
for($i=0;$i<=15;$i++)
	$angle_array[] = $i;

$captcha = [];
for($i=0;$i<mb_strlen($_SESSION['captcha']);$i++)
	$captcha[] = $_SESSION['captcha'][$i];
//echo Debug::d($captcha,'');

$x = 0; $y = 20; 
foreach($captcha as $k => $v){
	$angle = $angle_array[rand(0,count($angle_array)-1)]; 

	imagettftext($img['resourse'], 17, -$angle, $x, $y, $rand_color_converted, $dir, $v);	
	$x+=13; //$y+=20;
	//echo $v . "<br>";
}

//
function draw_line($img, $color='')
{
	$img_line['x_offset'] = 5;
	$img_line['y_offset'] = 3;
	$img_line['x1'] = rand(0, $img_line['x_offset']);
	$img_line['y1'] = rand(0, $img['height']);
	$img_line['x2'] = rand($img['width']-$img_line['x_offset'], $img['width']);
	$img_line['y2'] = rand(0, $img['height']);

	if ($color !== ''){
		$mc = convert_hexcolor2dec($color);

		$img_line['color'] = imagecolorallocate($img['resourse'], $mc[0], $mc[1], $mc[2]); 
	}else{
		$img_line['color'] = imagecolorallocate($img['resourse'], 200, 100, 90); // red
	}	

	return imageline($img['resourse'], $img_line['x1'] , $img_line['y1'] , $img_line['x2'] , $img_line['y2'] , $img_line['color'] );
}
draw_line($img,$fixed_color);
draw_line($img,$fixed_color);
draw_line($img,$fixed_color);

//
header("Content-type: image/png");
imagepng($img['resourse']);

?>